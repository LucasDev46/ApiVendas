...
public class OrderService : IOrderService
{
    ...
    {
        var orders = await _unitOfWork._orderRepository.Get().Include(p => p.Client).Include(p => p.Products).ToListAsync();
        if (orders is null)
        {
            return null;
        }      
    ...
    {
        var order = await _unitOfWork._orderRepository.Get().Include(p => p.Client).Include(o => o.Products).FirstOrDefaultAsync(p => p.OrderId == id);
        if (order is null)
        {
            return null;
        }
    ...
    {
        var client = await _unitOfWork._clientRepository.GetByQuery(p => p.CNPJ == cnpj);
